...
// 'authorized' (real money moved); that message surfaces here unchanged,
// no need to duplicate the guard client-side.
export async function cancelBusinessReservation(offerId) {
  const { data, error } = await supabase.rpc('cancel_business_reservation', { offer_id_param: offerId });
  if (error) throw new Error(error.message);
  return data;
}
