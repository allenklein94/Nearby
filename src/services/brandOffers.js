...
export async function updateBusinessAddress(partnerId, address) {
  ...
  const { error } = await supabase.rpc('update_business_profile', {
    ...
    // (2026-09-06): update_business_profile's subcategory_param follows
    // category_param's own non-coalesce contract -- must always be
...
export async function updateBusinessProfile(partnerId, { name, description, address, logoUrl, category, attributes, cuisine, differentiator, subcategory, categories }) {
  ...
  const { error } = await supabase.rpc('update_business_profile', {
    ...
    // Intent engine vision, layer 2 (subcategory) first increment
    // (2026-09-06): same non-coalesce contract as category_param above --
...
export async function setBusinessBookingMode(partnerId, mode) {
  const { error } = await supabase.rpc('set_business_booking_mode', { partner_id_param: partnerId, mode_param: mode ?? null });
  if (error) throw error;
}
