...
export default function usePersonalization() {
  const [state, setState] = useState({ declared: [], behavior: {}, maturity: null });
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        const [{ data: profile }, events, attended] = await Promise.all([
          supabase.from('profiles').select('interests, created_at').eq('id', uid).single(),
        ...
        if (!cancelled) setState({ declared: canonicalizeInterests(profile?.interests), behavior, maturity });
      } catch {
