...
// Nearby Motion Language. A deliberately SMALL set of per-occasion micro-
// celebrations -- only the 5 named ones (birthday/anniversary/graduation/
// celebration/surprise). Every other occasion gets no animation at all, on
// purpose -- "not a giant confetti explosion every time," just enough to
// signal "you've entered a special planning experience."
export const OCCASION_SELECT_ANIMATIONS = {
  ...
  // A quick unlocked-to-locked snap, then the real mode name, rather than a
  // full theme change (a much bigger, separate redesign this doesn't ask for).
...
// than shown as a frozen mid-burst frame.
const REDUCED_HOLD_MS = 450;

export default function OccasionAnimation({ triggerKey, onDone }) {
