-- 20270186 (first_date; also re-checks 20270182): the new occasions pass every gate; junk still fails; single overloads. Rolled back.
begin;
do $$
...
begin
  ...
  assert _occasion_noun('first_date') = 'First Date' and _occasion_emoji('first_date') is not null, 'first_date noun/emoji';
  ...
  perform create_business_request(raw_text_param => 'live-verify 3', latitude_param => 40.0, longitude_param => -75.0, category_param => 'Restaurants', party_size_param => 2, budget_max_param => 40, occasion_param => 'first_date');
  ...
  perform set_business_priority_occasions(partner, array['fundraiser','bachelor_bachelorette','first_date']);
