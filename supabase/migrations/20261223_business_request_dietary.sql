-- Structured dietary needs: a closed vocabulary the CONSUMER picks per request (never inferred, never free text), so a business
-- that must fulfil a meal can see e.g. "Nut allergy, Vegan" without any typed sentence crossing the boundary.
-- Visible to a business only through get_business_opportunities (i.e. only for a request it has an offer on) and only when non-empty.
-- Deliberately NOT in business_safe_request_summary or any push body (sensitive; lock screens).

alter table public.business_requests add column if not exists dietary text[] not null default '{}';
alter table public.business_requests drop constraint if exists business_requests_dietary_check;
alter table public.business_requests add constraint business_requests_dietary_check
  check (dietary <@ array['vegetarian','vegan','gluten_free','dairy_free','nut_allergy','shellfish_allergy','halal','kosher']::text[] and cardinality(dietary) <= 8);

drop function if exists public.create_business_request(raw_text_param text, latitude_param double precision, longitude_param double precision, category_param text, party_size_param integer, budget_min_param integer, budget_max_param integer, date_param date, time_window_start_param time without time zone, time_window_end_param time without time zone, radius_miles_param double precision, submission_id_param uuid, preferred_availability_id_param uuid, attributes_param text[], cuisine_param text, occasion_param text, preferred_package_id_param uuid, experience_level_param text, surprise_mode_param boolean, shared_interests_param text[]);
...
begin
  ...
  insert into business_requests (
    ...
    experience_level, surprise_mode, shared_interests, dietary
  ) values (
...
revoke all on function public.create_business_request from public, anon;
grant execute on function public.create_business_request to authenticated;
