-- "Demand near you" weekend framing: category, group and occasion rows gain `weekend_count` = distinct people whose REQUEST
-- is dated in the coming Fri-Sun (the same weekend definition as get_occasion_demand_for_partner). Requests only (searches
-- have no date, so "of them this weekend" is always a subset of the row's people). Floored at 5 distinct people on its own
-- and withheld when 0 < people_count - weekend_count < 5, so subtracting two shown numbers can't expose a hidden group.
-- Absent means absent, never 0. Signature and every existing field unchanged (single overload).
