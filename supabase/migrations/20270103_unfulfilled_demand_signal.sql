-- "Unfulfilled demand": for each category row on "Demand near you", also say how many of those people still have an
-- open request nobody has offered on (unfulfilled_count) and how many active businesses within 15 miles offer the
-- category (supply_count). Same privacy rules as the rest of this function: unfulfilled_count is returned only when
-- >= 5 DISTINCT people are behind it (else null, and so is supply_count, which is only computed for those rows);
-- no person ids, raw text or coordinates. Requests only (searches have no fulfilled/unfulfilled state).
