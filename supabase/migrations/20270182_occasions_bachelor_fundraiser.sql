-- Two new occasions (owner request, 2026-09-21): bachelor_bachelorette ("Bachelor/Bachelorette") and fundraiser.
-- Every occasion vocabulary gate is widened together in this one migration (the repo's rule after the list drifted
-- before, see 20261031): six table CHECKs and six functions, each patched from its LIVE definition. Deliberately NOT
-- touched: business_availability_bundle_occasion_check (bundles keep their own narrow list; new experience templates
-- would be a separate feature). Festival / networking / community event stay formats (category tags), not occasions.

alter table public.brand_partners drop constraint if exists brand_partners_priority_occasions_check;
alter table public.brand_partners add constraint brand_partners_priority_occasions_check CHECK ((priority_occasions <@ ARRAY['birthday'::text, 'anniversary'::text, 'date_night'::text, 'celebration'::text, 'casual_hangout'::text, 'business_meal'::text, 'family_gathering'::text, 'graduation'::text, 'baby_shower'::text, 'engagement'::text, 'wedding'::text, 'housewarming'::text, 'new_job'::text, 'promotion'::text, 'retirement'::text, 'achievement'::text, 'moving'::text, 'farewell'::text, 'reunion'::text, 'welcome'::text, 'holiday_gathering'::text, 'bachelor_bachelorette'::text, 'fundraiser'::text, 'milestone'::text, 'life_event'::text, 'other'::text]));
...
begin
  ...
  if occasion_param is not null and occasion_param not in (
    ...
    'farewell', 'reunion', 'welcome', 'holiday_gathering', 'bachelor_bachelorette', 'fundraiser', 'milestone',
    'life_event', 'other'
  ) then
