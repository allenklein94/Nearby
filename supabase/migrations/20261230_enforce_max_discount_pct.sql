-- Enforce the business's max_discount_pct (fulfillment policy) server-side.
--
-- Until now max_discount_pct was stored-only and offers carried NO structured
-- discount, only offer_type='discount' + a price + free text, so there was
-- nothing to compare against the cap. This adds a real discount_pct to an
-- offer and to an availability posting, and ONE chokepoint -- a BEFORE
-- INSERT/UPDATE trigger on business_request_offers -- that every path that
-- makes an offer 'offered' goes through (manual submit_business_offer, the
-- screening-approval branch, availability postings, availability/package/
-- policy auto-offers). The cap is read from the business's ACTIVE
-- business_fulfillment_policies row; no policy / inactive / null cap = no
-- limit (the pre-existing behavior; no new default).
--
--   * cap set + offer_type='discount' + no stated discount_pct -> rejected:
--     the cap cannot be verified, so a percentage is required.
--   * discount_pct > cap -> rejected (any offer_type).
--   * MANUAL paths raise a clear error. AUTO-created offers (those carrying
--     availability_id / package_id) are silently NOT created instead, so one
--     over-cap posting can never abort a request fan-out; the request just
--     stays pending for a manual response. Availability-sourced offers
--     inherit discount_pct from their posting.
--   * post_business_availability also checks at posting time.

alter table public.business_request_offers
  add column if not exists discount_pct numeric(5,2);
alter table public.business_request_offers
  drop constraint if exists business_request_offers_discount_pct_check;
alter table public.business_request_offers
  add constraint business_request_offers_discount_pct_check
  check (discount_pct is null or (discount_pct >= 0 and discount_pct <= 100));

alter table public.business_availability
  add column if not exists discount_pct numeric(5,2);
alter table public.business_availability
  drop constraint if exists business_availability_discount_pct_check;
alter table public.business_availability
  add constraint business_availability_discount_pct_check
  check (discount_pct is null or (discount_pct >= 0 and discount_pct <= 100));

create or replace function public._discount_cap_violation(
  partner_id_param uuid, offer_type_param text, discount_pct_param numeric
)
returns text
language plpgsql
stable
...
begin
  ...
  where partner_id = partner_id_param and active;

  if v_cap is null then
    return null;
  end if;
  if discount_pct_param is null then
    if offer_type_param = 'discount' then
      return 'Enter the discount percentage -- your policy caps discounts at ' || trim_scale(v_cap)::text || '%.';
    end if;
    return null;
  end if;
  if discount_pct_param > v_cap then
    return 'This discount (' || trim_scale(discount_pct_param)::text || '%) is above your maximum discount of '
      || trim_scale(v_cap)::text || '%. Lower it, or raise the cap in your policy.';
  end if;
  return null;
end;
$function$;
revoke all on function public._discount_cap_violation(uuid, text, numeric) from public, anon, authenticated;
grant execute on function public._discount_cap_violation(uuid, text, numeric) to service_role;

create or replace function public.enforce_offer_discount_cap()
returns trigger
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  if TG_OP = 'UPDATE' and OLD.status = 'offered' then
    return NEW;
  end if;

  if NEW.discount_pct is null and NEW.availability_id is not null then
    select discount_pct into NEW.discount_pct from business_availability where id = NEW.availability_id;
  end if;

  v_msg := public._discount_cap_violation(NEW.partner_id, NEW.offer_type, NEW.discount_pct);
  if v_msg is null then
    return NEW;
  end if;

  if NEW.availability_id is not null or NEW.package_id is not null then
    return null;
  end if;
  raise exception '%', v_msg;
end;
$function$;
revoke all on function public.enforce_offer_discount_cap() from public, anon, authenticated;

drop trigger if exists business_request_offers_enforce_discount_cap on public.business_request_offers;
create trigger business_request_offers_enforce_discount_cap
  before insert or update on public.business_request_offers
  for each row execute function public.enforce_offer_discount_cap();

drop function if exists public.submit_business_offer(uuid, text, text, numeric, timestamptz, uuid, text, text, text, text[], boolean);

CREATE OR REPLACE FUNCTION public.submit_business_offer(request_id_param uuid, offer_type_param text, offer_description_param text, offer_price_param numeric DEFAULT NULL::numeric, proposed_time_param timestamp with time zone DEFAULT NULL::timestamp with time zone, experience_id_param uuid DEFAULT NULL::uuid, media_path_param text DEFAULT NULL::text, media_type_param text DEFAULT NULL::text, offer_title_param text DEFAULT NULL::text, included_items_param text[] DEFAULT '{}'::text[], price_is_per_person_param boolean DEFAULT false, discount_pct_param numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
revoke all on function public.submit_business_offer(uuid, text, text, numeric, timestamptz, uuid, text, text, text, text[], boolean, numeric) from public, anon;
grant execute on function public.submit_business_offer(uuid, text, text, numeric, timestamptz, uuid, text, text, text, text[], boolean, numeric) to authenticated;

drop function if exists public.post_business_availability(text, text, text, text, numeric, integer, timestamptz, timestamptz, double precision, text, text[]);
...
revoke all on function public.post_business_availability(text, text, text, text, numeric, integer, timestamptz, timestamptz, double precision, text, text[], numeric) from public, anon;
grant execute on function public.post_business_availability(text, text, text, text, numeric, integer, timestamptz, timestamptz, double precision, text, text[], numeric) to authenticated;
