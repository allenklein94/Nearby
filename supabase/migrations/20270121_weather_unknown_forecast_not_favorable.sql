-- Weather: an unknown forecast is unknown, never favorable (2026-09-20).
--
-- get_weather_result defaulted the forecast-derived signals to
-- outdoor_favorable=true / rain_risk='low' / heat=false / cold=false when
-- the forecast leg had not resolved (or returned an API error body with
-- no `list`). Those are now NULL (unknown). Same signature and return
-- type as 20260822, so CREATE OR REPLACE replaces it in place (still one
-- overload). The current-conditions label/detail are unchanged.
