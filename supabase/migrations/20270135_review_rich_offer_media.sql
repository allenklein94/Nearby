-- A held offer response that a reviewer later approves must publish the same rich fields the direct path does
-- (poster image for a video, redemption instructions). Body unchanged from the live definition except those two columns.
