-- "Supply first": a business posts availability for a SCHEDULED window (e.g. Friday 6-8 PM) and sees,
-- before committing, how many people nearby have an open request that window would match.
--
-- get_availability_demand_preview mirrors post_business_availability's matching predicates exactly
-- (open, unexpired request; capacity >= party size; category; date inside the window; time window overlap;
-- distance <= least(request radius, posting radius)) but counts DISTINCT PEOPLE (requester_id) and, like
-- every business-facing demand aggregate, returns NULL below demand_min_people() (5). Owner-only, count
-- only -- no ids, no text. Posting itself is unchanged (it already offers to matches); the preview just
-- makes it demand-led. The screening-approval branch now honors an explicit startsAt/endsAt from the
-- snapshot (a scheduled window has no stale-window problem), and refuses one that has already passed.

create or replace function public.get_availability_demand_preview(
  category_param text,
  starts_at_param timestamptz,
  ends_at_param timestamptz,
  capacity_param integer default null,
...
begin
  ...
  if v_lat is null or v_lng is null then
    return jsonb_build_object('people', null, 'floor', demand_min_people());
  end if;

  select count(distinct br.requester_id)::integer into v_people
  ...
  where br.status = 'open'
    ...
    and (br.date is null or br.date between starts_at_param::date and ends_at_param::date)
    and (
  ...
  return jsonb_build_object(
    'people', case when v_people >= demand_min_people() then v_people else null end,
    'floor', demand_min_people()
  );
end;
$function$;

revoke all on function public.get_availability_demand_preview(text, timestamptz, timestamptz, integer, double precision) from public, anon;
grant execute on function public.get_availability_demand_preview(text, timestamptz, timestamptz, integer, double precision) to authenticated;
