-- "Demand near you" gains two structured dimensions per category row, from stored fields only (never free text/AI):
--   when_day + when_period : the single most-requested (weekday, time-of-day) cell, from business_requests.date and
--                            time_window_start. A request with no date OR no time window contributes nothing (no time is
--                            ever inferred). Period: before 12:00 morning, 12:00-16:59 afternoon, 17:00+ evening.
--   outdoor                : true when people asked for the structured 'outdoor_seating' attribute.
-- Privacy: each fact has its own floor (>= 5 DISTINCT people in that exact cell) and a COMPLEMENT rule against the
-- row's visible people_count: if 0 < people_count - cell_people < 5 the fact is withheld, otherwise subtracting the
-- shown numbers would expose a hidden group of fewer than 5 (e.g. "only 3 did NOT ask for Friday"). Below the floor the
-- fact is simply absent -- never a 0 or a partial count. Requests only (searches carry no date/time/attributes).
-- The two facts are independent floored facts, not a claim that the same people did both.
