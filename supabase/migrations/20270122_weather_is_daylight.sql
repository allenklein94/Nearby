-- Weather: day/night from the current-conditions payload (2026-09-20).
--
-- Adds is_daylight (true/false from OpenWeather's own day/night icon code,
-- NULL when unknown) so a "good outdoor window" is only ever claimed in
-- daylight. Return shape changes, so the old function is dropped first
-- (single overload). Everything else is identical to 20270121.
