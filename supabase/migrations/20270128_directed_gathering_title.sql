-- A host who deliberately sends a gathering to ONE specific business (is_directed) shares the gathering's
-- title with that business (owner decision 2026-09-20). Broadcast/fan-out requests still carry no title.
    ...
    select
      ...
      jsonb_build_object(
        ...
        'gatherings', case when g.id is not null then jsonb_build_object(
          ...
          'title', case when bro.is_directed then g.title else null end,
