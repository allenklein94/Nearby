...
-- project_intent_engine_vision). Closes the one deliberately-deferred gap
-- from the first subcategory increment: brand_partners.subcategory has
-- been a durable identity field (display + a picker default) but was
-- never actually returned to the intent resolver, so it fed no real
-- matching signal yet. A business's own declared subcategory (e.g.
-- "Coffee" under food_drink) is the SAME leaf-tag vocabulary the ask's own
-- `category` param already is (gatherings.interest_tag / business_
-- requests.category / business_availability.category all already share
-- it) -- comparing them directly is apples to apples, no mapping needed.
--
-- This is a genuinely stronger signal than the existing category_param
-- match against ba.category (the per-posting tag): a business can post an
-- untagged/differently-tagged availability row and still be a real match
-- because its own standing identity says "I'm specifically a coffee shop"
-- -- same "real signal, flat bonus, never a filter" shape every other
-- bonus here already follows (occasionBonus, accommodatesPartyTypeBonus,
-- attributeAndCuisineBonus).
--
-- RETURNS TABLE's column list is changing again, so (per this same
-- migration series' own now-twice-learned lesson) a real drop is required
-- -- CREATE OR REPLACE cannot change it, and would in fact just create a
-- second overload if attempted without the drop, exactly like the
-- non-RETURNS-TABLE update_business_profile overload trap two migrations
-- back. Every other line is byte-for-byte the live version (confirmed via
