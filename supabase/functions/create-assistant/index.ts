...
// per 20260927_business_semantic_tags_expansion.sql / businessAttributes.js's
// own header comment (memory project_intent_engine_vision). Keep this
// array in sync with that migration's CHECK-constraint array and with
// businessAttributes.js's BUSINESS_ATTRIBUTE_OPTIONS if either ever changes.
const VALID_ATTRIBUTES = [
  ...
  'wheelchair_accessible', 'accessible_parking', 'accessible_restroom', 'service_animal_friendly', 'stroller_friendly', 'family_seating', 'kid_menu', 'pet_friendly', 'romantic', 'catering', 'lively', 'trendy', 'relaxed', 'social', 'cozy', 'professional',
];
...
// Item 61 ("Celebrate Something", CLAUDE.md) widened this with 7 life-event
// values (graduation through milestone) -- mirrors
// 20261016_celebrate_occasion_vocabulary_expansion.sql exactly.
const VALID_OCCASIONS = [
  ...
  'engagement', 'housewarming', 'promotion', 'farewell', 'milestone', 'bachelor_bachelorette', 'fundraiser', 'first_date', 'self_care', 'networking', 'vacation', 'other',
];
...
If intent is "gathering" or "community", also extract a short best-effort title (a few words, based only on what they wrote). If intent is "business_partner", also extract the business name if one was mentioned, or null if not.

Regardless of intent (including "unclear" — many real, valid requests, like "can someone find me a good place for dinner?", don't describe hosting or starting anything, so they correctly classify as "unclear", but Nearby should still search relevantly for them), also extract these when the text gives a real clue (used only to help find something that already exists — never to create or publish anything):
- category: one value from this exact list: ${JSON.stringify(VALID_CATEGORIES)} (pick the closest match to what the text is actually about, e.g. "a good place for dinner" implies "Foodie", "want to shoot some hoops" implies "Sports"), or null if nothing in the text implies one. Never guess a category from tone alone -- only pick one when the text genuinely names or clearly implies an activity/topic.
- partySize: a whole number of people, always including the person asking, if mentioned. "My girlfriend and I" is 2. "8 of us" is 8. "For two people" or "me and one friend" is 2. "With two friends" or "me and two friends" means the asker plus two others, so it's 3 -- when the text names a number of *other* people (friends, guests) without saying that count is the total, add 1 for the asker. "Get 6 people together" or "a group of 6" is 6 (already a total headcount, not additive). Only set a number when the text gives a real clue; otherwise null.
- dateWindow: one of ${JSON.stringify(VALID_DATE_WINDOWS)} — pick the closest match to what they wrote, using this exact mapping: "right now"/"immediately"/"right away"/"as soon as possible" is "now" (they mean *this specific moment*, not just sometime today). "tonight"/"this evening" is "tonight". Plain "today" with no urgency implied is "today". "this weekend" is "weekend", and so is a plain "Saturday" or "Sunday" with no other timing word (the closest real bucket this list has for either day, even though it's a two-day bucket) -- "now" and "tonight" are genuinely different asks — never pick "now" just because "tonight" would also be a reasonable guess, and never pick "tonight" for something that clearly means immediately. "flexible" if no timing was mentioned at all. A plain weekday name (Monday-Friday) with nothing else to place it in today/tomorrow/this weekend has no real bucket in this list -- leave dateWindow null rather than force it into the wrong one. Never guess a specific date or clock time — only pick from this exact list.
...
- partyType: one of ${JSON.stringify(VALID_PARTY_TYPES)} if the text implies who this is for (e.g. "just me"/"solo" is "solo", "me and my friends" is "friends", "a big group" is "groups", "a date"/"with my partner"/"with my wife"/"with my husband"/"with my girlfriend"/"with my boyfriend" is "date", "coworkers"/"colleagues"/"my team"/"the office" is "coworkers", "family"/"with my kids"/"with the kids"/"my parents" is "family", "meet new people"/"make new friends"/"people I don't know" is "new_people"; a headcount alone such as "8 people" is NOT a party type -- it is partySize), or null if no such signal was mentioned at all. Never guess.
- attributes: an array of zero or more values from this exact list: ${JSON.stringify(VALID_ATTRIBUTES)} -- only include one when the text genuinely names that specific quality (e.g. "outdoor seating"/"patio" implies "outdoor_seating", "for a date"/"romantic" implies "date_friendly", "for the kids" implies "kid_friendly", "somewhere quiet" implies "quiet", "casual" implies "casual", "somewhere nice/upscale/fancy" implies "upscale", "lively"/"energetic"/"buzzing" implies "lively", "trendy"/"hip"/"stylish" implies "trendy", "relaxed"/"laid-back"/"chill" implies "relaxed", "a social spot" implies "social", "cozy" implies "cozy", "somewhere professional"/"a client meeting" implies "professional", "live music" implies "live_music", "for a big group" implies "group_friendly", "good coffee"/"specialty coffee" implies "specialty_coffee", "somewhere to work"/"wifi"/"bring my laptop" implies "laptop_friendly", "can bring my dog" implies "dog_friendly", "on the water"/"waterfront"/"lake view" implies "waterfront", "open late"/"late night" implies "late_night", "board games"/"game night" implies "board_game_friendly", "good for photos"/"photogenic"/"instagrammable" implies "photography_friendly", "bookstore"/"book club" implies "book_lovers", "a craft workshop"/"make my own" implies "craft_friendly", "a workout"/"the gym" implies "fitness_focused"). A hobby or interest mentioned in the text (photography, board games, reading, fitness, etc.) should map to its matching attribute here even when it isn't itself the category — this lets the match consider any business that says it's good for that hobby, not just businesses whose whole category is that hobby. Never guess -- an empty array is the common, correct answer when nothing specific was named.
...
- occasion: one value from this exact list: ${JSON.stringify(VALID_OCCASIONS)} if the text names a real reason for the plan, not just what/when/where. "birthday"/"turning 30"/"her birthday" is "birthday". "anniversary" is "anniversary". "date"/"date night"/"anniversary dinner for two" (when not a literal anniversary) is "date_night". "just celebrating"/"celebration" with no more specific reason named is "celebration". "graduation"/"graduating"/"grad party" is "graduation". "baby shower" is "baby_shower". "engagement"/"engagement party"/"just got engaged" is "engagement". "housewarming"/"new place"/"moved into a new house" is "housewarming". "promotion"/"new job"/"got promoted"/"first day at work" is "promotion". "farewell"/"going away party"/"last day"/"moving away" is "farewell". "milestone"/"big achievement"/"retirement" is "milestone". "bachelor party"/"bachelorette party"/"bachelorette weekend"/"hen party" is "bachelor_bachelorette". "fundraiser"/"charity event"/"benefit dinner" is "fundraiser". "first date"/"meeting someone for the first time"/"first meet" is "first_date" (a returning couple's dinner is "date_night", not "first_date"). "self-care day"/"spa day"/"treat myself"/"pamper" is "self_care". "networking"/"networking event"/"meet other professionals" is "networking". "vacation"/"holiday trip"/"getaway"/"trip to" is "vacation". "just hanging out"/"nothing special" is "casual_hangout". "work dinner"/"client meeting"/"team lunch" is "business_meal". "family reunion"/"visiting family" is "family_gathering". Use "other" only when a real occasion is clearly implied but doesn't fit any of the above -- leave this null (not "other") when no occasion is implied at all. Never guess an occasion from the category/activity alone (e.g. "dinner" alone does not imply "date_night" or "celebration" -- only pick one when the text itself gives a real reason).
