-- Adds an "included units, then overage" step to per-redemption and hybrid
-- contracts, so a partner can get e.g. "100 redemptions included, $0.75
-- each after that" instead of paying the per-redemption rate from
-- redemption #1. flat_monthly and custom contracts ignore this column —
-- there's no per-unit rate for them to apply it to.

alter table public.partner_contracts
  add column if not exists included_units integer not null default 0;

alter table public.partner_contracts
  drop constraint if exists partner_contracts_included_units_non_negative;
alter table public.partner_contracts
  add constraint partner_contracts_included_units_non_negative check (included_units >= 0);

-- ---------------------------------------------------------------------
-- generate_monthly_invoices() — same signature, just applies
-- included_units before multiplying by redemption_fee.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- get_partner_billing_estimate() — return shape changes (adds
-- included_units/billable_count), so drop + recreate rather than
-- create or replace.
-- ---------------------------------------------------------------------
