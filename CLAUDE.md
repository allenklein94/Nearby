...
- **Weekend framing + "Your business matches N" (2026-09-19, migration `20270111`, verified live rolled-back via `scripts/live-verify/demand-weekend-framing.js`; APPLY TO PROD PENDING the user -- see status below):** category, group and occasion rows of `get_partner_demand_signals` gain `weekend_count` = distinct people whose REQUEST is dated in the coming Fri-Sun (same definition as `get_occasion_demand_for_partner`); requests only (searches have no date), floored at 5 on its own and withheld when 0 < people_count - weekend_count < 5; card copy "N of them this weekend"; absent means absent. The card also opens with "Your business matches N open requests right now -> View opportunities", the owner's OWN open pending opportunities (first-party, not floored). Deliberately NOT built: a qualitative "High demand" label (would need an invented threshold). Jest 904; web export regenerated; not browser-tested.
