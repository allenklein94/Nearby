...
**Business website == app parity (2026-09-19, requirement: an owner can do receive -> review -> offer -> accept -> manage -> decline -> update availability -> manage reservations from either surface).** Both run the same `BusinessDashboardScreen`, so parity lives in the platform shell. Audit found ONE real break: react-native-web's `Alert.alert` is a no-op, so on the website every confirmation (Cancel Reservation, delete/pause package, remove experience), validation message and error silently vanished -- Cancel Reservation could not run at all. Fixed by `src/utils/webAlert.js` (browser alert/confirm adapter, installed in `App.web.js`; native untouched). Date/time inputs were already covered (`PlatformDateTimeInput`); redemption confirmation works via the 6-digit code entry (only the camera scanner is native). `businessWebParityGuard.test.js` guards the seams. **Disclosed gaps (web = "open the app"):** hosting gatherings/communities (`CreateGathering`, `GatheringDetail`, `CreateCommunity`, `CommunityDetail`), the QR camera scanner, Stripe Connect payout setup (hidden on web), and push (web gets the email fallback, inert until Resend is connected). Not browser-tested; web export regenerated; Jest 854.
