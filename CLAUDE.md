...
## Aug 17 2026 — Business Partner acquisition experience: plan only, per direct instruction — NOT YET BUILT

Written before implementation, same restart-safety convention as every other plan-first section
in this file — **nothing in this section has been built.** The user forwarded two documents from
an external AI in one message and explicitly asked to plan this out in this file before building
anything ("there's a lot to digest here... several audits were given... maybe you can combine
those into one"). Both documents are combined into one plan below, checked against this
codebase's real current state via a dedicated read-only research pass (not assumed from memory
or from either external doc's own claims) before locking anything in.

### What the external doc actually said, compressed

Two distinct pieces, worth keeping separate since they call for different things:

1. **A strategic reflection on Nearby's ceiling — not a spec, no code implication on its own.**
   Captured here the same way this file's own "Nearby 2.0 Vision"/"V3/V4 strategic vision"
   sections captured outside strategic input before: read, recorded, not auto-scheduled. Core
   argument: Nearby's real differentiation isn't any single feature, it's the connective tissue
   between consumer intent, plans/gatherings, communities, businesses, offers, location, and
   real-time demand — a "local activity graph" that becomes harder to replicate the more real
   activity flows through it. The explicit warning: adding features doesn't build that graph,
   real usage does — "50 beautifully connected features with 2,000 highly active local users"
   beats "500 features with 200 active users." The proposed proof-of-concept is a single
   concentrated geographic market showing real consumer return-usage, real business
   participation, and businesses referring other businesses. **Nothing here is a build item** —
   it's already consistent with this file's own standing Feature Freeze framing (stabilize/prove
   the core loop before building more) and with the repeatedly-stated "Real-World Validation is
   the single biggest real risk" line in the honest scorecard above; this doc restates that
   argument from a business/strategy angle, doesn't contradict anything already decided.
2. **A concrete, detailed implementation brief**: build a self-service "Business Partner"
   acquisition + onboarding experience — a landing page that explains Nearby to a business owner
   in ~5-10 seconds, shows what they get, shows a demo, states pricing (free to start), then
   funnels into search → claim an existing business record ("Is this your business?") → verify
   ownership → confirm/complete profile → publish → first offer → dashboard → analytics,
   instrumented end-to-end, using the app's real data model throughout (no mock/duplicate data),
   reusing the existing design system. The brief itself prescribes an audit-first, milestone-by-
   milestone build with a stop-and-verify gate after each milestone, plus three adversarial
   review passes at the end (a hostile business owner with 60 seconds and no context; a consumer-
   connectivity trace; a security pass attempting unauthorized claim/edit access).

### Phase 0 audit — DONE this pass (read-only, no code changed), closing the brief's own "audit first" requirement

Read the actual current code rather than assuming either doc's claims. Full findings, condensed:

1. **No web presence exists anywhere in this repo.** `app.json`/`package.json` have no `web`
   target, no web build script, no `landing/`/`marketing/` directory. `docs/index.html`/
   `privacy.html`/`terms.html`/`track.html` exist at the repo root but are a separate, unrelated
   static legal micro-site (privacy policy, terms, a Leaflet live-location viewer for date-
   safety-check-in) with no shared design system, no Expo involvement. **This is a mobile-only
   Expo app** — the brief's own core assumption ("hand a business owner a URL and QR code") has
   nowhere to live today without a new decision about what that surface even is.
2. **There is no "claim an existing business" concept anywhere, and no external business data is
   ever imported.** `BusinessPartnerApplyScreen.js` is a from-scratch application form — zero
   pre-fill, zero lookup against any pre-existing record. `approve_business_partner_request()`
   (latest def, `20260811_business_partner_category.sql`) always `insert`s a brand-new
   `brand_partners` row on approval, and in the same transaction sets the *approving requester's
   own* `profiles.managed_partner_id` — meaning every `brand_partners` row that has ever existed
   in this schema was born already-owned, at creation time. There is no such thing as an
   "unclaimed" business record sitting in this database waiting to be claimed, and nothing in
   this codebase has ever imported real external business listings (Google Places is used live/
   read-only elsewhere — `PlacesScreen.js`/Discover — never persisted into `brand_partners`).
   **The brief's central "search for your business → Is this you? → Claim It" interaction has
   nothing to reuse and cannot work against today's data model as-is** — this is the single
   biggest reality gap between the brief and the app, and it's a real product decision, not a
   wiring gap (see Decision 2 below).
3. `getActivePartnersByName()`/`RequestBusinessPartnerScreen.js` search only already-approved,
   active partners, for the opposite direction (a gathering/community host picking a business to
   propose sponsorship to) — not reusable for a claim flow.
4. **The business dashboard and its analytics are already real, substantial, and mostly done.**
   `BusinessDashboardScreen.js` (1913 lines, 6 tabs: Dashboard/Gatherings/Community/Requests/
   Insights/Business) already has every analytics RPC the brief asks for:
   `get_business_dashboard_stats`, `get_business_growth`, `get_business_member_gathering_history`,
   `get_business_visit_frequency`, `get_business_insights`, `get_business_top_members`,
   `get_business_follower_count`, `get_partner_avg_response_time`, `get_partner_offer_reputation`,
   `get_marketplace_reliability_rankings`, `get_aggregated_demand_for_partner`,
   `get_home_nudge_stats`, plus real per-customer CRM notes (`business_customer_notes`). **The
   brief's Milestones 4 ("business profile"), 6 ("dashboard"), and 7 ("analytics") are ~90%
   already built** — this is not a from-scratch build, most of the brief's own scope already
   exists.
5. **Offers are already real, full CRUD** — `brand_offers` (title/description/reward_type/
   redemption limits/target interest/group-unlock) already creatable/editable/toggleable from the
   dashboard today (`createBusinessOffer`/`toggleOfferActive`).
6. **Deep linking is minimal — one route only, no business link, no QR.** `RootNavigator.js`'s
   entire `linking.config.screens` object has exactly one entry (`GatheringDetail:
   'gathering/:gatheringId'`). No `nearby://business/:id` deep link exists. No QR code library
   exists anywhere in `package.json` — both would need to be built new.
7. **The existing referral system (`services/referrals.js`/`InviteFriendsScreen.js`) is consumer-
   to-consumer only** ("invite a friend to the app") — confirmed no business-referring-business
   mechanism exists anywhere, matching the brief's own "businesses voluntarily referring other
   businesses" language, which is aspirational, not yet real.
8. **This app already has a well-established, reusable pattern for exactly the kind of funnel
   instrumentation the brief asks for** — `intent_submissions`/`intent_outcomes`/
   `home_nudge_events` (plain owner-scoped tables, no RPC needed for the write side, fire-and-
   forget client writes, one admin-gated aggregate RPC for the rollup, rendered on
   `MarketValidationScreen.js`). A business-acquisition funnel (landing viewed → demo opened →
   CTA clicked → search started → claim started/completed → verification → profile completed →
   published → first offer → first consumer interaction → dashboard viewed) should follow this
   exact shape, not invent a new one.
9. **`BusinessProfileScreen.js`** (the existing public, consumer-facing business profile) is a
   real, working reference for "here's what your business looks like to a consumer" — header/
   follow/message, "What People Say" (real aggregated feedback), Photos (sourced from the
   business's own gathering cover photos — `brand_partners` has no dedicated photo-gallery
   field), Perks (real active offers), Upcoming Gatherings (public only). Any landing-page
   mockup of "here's your business in the app" should be built to genuinely match this screen,
   not a fabricated design.
10. **Design system**: `src/theme.js` (`colors`/`typography`/`spacing`/`radius`/`shadow` +
...
3. [x] **Business deep link + QR — DONE.** Built exactly per the plan's own decision rule
   ("landing... depending on whether the caller already manages that business") — one real,
   disclosed correction to the plan's own text along the way: it named "the apply flow" as the
   non-owner branch, which doesn't fit the deep link's own real shape (a `nearby://business/:id`
   link only ever exists for an *already-real* `brand_partners` row — there's no
   "business doesn't exist yet" case for it to route around, since it's only ever generated by
   this same milestone's new "Share Your QR Code" surface, gated on `selectedPartner` already
   existing). Built the branch the rule actually describes: the business's own owner lands on
   `BusinessDashboard`, everyone else lands on `BusinessProfile` (`partnerId`) — a stranger, a
   customer, or the business owner opening someone *else's* QR code all correctly get the public
   consumer view.
   `nearby://business/:partnerId` was deliberately **not** added to `RootNavigator.js`'s
   `linking.config.screens` (unlike the existing `GatheringDetail` entry) — that config is a
   static path→screen table and can't express a branch that depends on who's opening the link,
   so NavigationContainer's own built-in URL resolution isn't the right mechanism here. Instead,
   reused and extended the *same* `PENDING_GATHERING_LINK_KEY` stash-then-consume mechanism this
   file already built to solve the harder half of this problem (a link opened before the
   recipient is even signed in, so the target screen doesn't exist in the tree yet to navigate
   to) — a parallel `PENDING_BUSINESS_LINK_KEY`, stashed by the same always-on
   `Linking.addEventListener`, consumed once `session && profileComplete` go true via a new
   shared `resolveAndNavigateToBusiness(partnerId)` (calls the existing `getMyManagedPartner()`,
   compares its `id` against the link's target, navigates accordingly). **One real gap the
   gathering link never had to solve, found and closed in this same pass**: gathering links don't
   need this because `GatheringDetail` is registered directly in `linking.config.screens`, so
   NavigationContainer's own subscription keeps resolving a warm (already-authenticated,
   already-running) tap for free, for the lifetime of the app — the business link's stash/consume
   effect only ever fires once, on the `session`/`profileComplete` transition, so a warm tap
   arriving after that transition already happened would otherwise be silently stashed and never
   consumed. Closed with a second, dedicated `useEffect` (deps `[session, profileComplete]`) that
   subscribes its own live `Linking.addEventListener` only while the authenticated stack actually
   exists, resolving a warm tap immediately rather than waiting on a stash it would never read
   again — unsubscribed automatically the moment `session`/`profileComplete` changes.
   **QR**: added `react-native-svg`+`react-native-qrcode-svg` (via `npx expo install`, so the
   SDK-54-compatible native module version was picked automatically, not guessed) — neither
   existed in this codebase before this pass, confirmed via the Phase 0 audit's own finding ("No
   QR code library exists anywhere in `package.json`"). New "📱 Share Your QR Code →" link on
   `BusinessDashboardScreen.js`'s Dashboard tab, right under the existing "👀 View Public Profile"
   link, gated on `selectedPartner` the same way — opens a real Modal rendering a live `QRCode`
   for `nearby://business/{the business's own real id}` plus a "Share Link" button reusing the
   exact same `Share.share({message, url})` shape `GatheringConfirmationScreen.js`'s "Share
   Gathering" action already established, not a second sharing convention.
   Verified via a full `npx expo export --platform ios` (clean, no bundling errors — 2186
   modules, 311 more than the prior 1875 baseline, entirely from `react-native-svg`'s own
   dependency tree plus `react-native-qrcode-svg`, both genuinely new). No schema/RPC change this
   milestone, so no live-production verification was needed — pure client-side navigation/UI.
   **Not done, same standing gap as everywhere else in this file**: no manual simulator/device
   run-through — next session should confirm, on a real device: tapping a `nearby://business/:id`
   link before signing in correctly lands on the right screen once signed in; tapping the
   identical link while already signed in and using the app (warm tap) lands correctly without
   needing to background/reopen the app; the business owner's own QR code correctly opens their
   *own* dashboard when scanned from their own second device, and correctly opens the public
...
6. **End-to-end verification**: a real test business run through the complete funnel (search/
   ...
   **All 5 previously-unfired funnel steps are now real, wired, and verified — DONE.** While
   preparing this verification, found 5 of the 11 real named funnel steps in
   `business_acquisition_events` (`profile_completed`/`published`/`first_offer_created`/
   `first_consumer_interaction`/`dashboard_viewed`) were defined in the table's own CHECK
   constraint and already read by `get_business_acquisition_funnel_stats()`, but never actually
   fired anywhere in client or server code — Milestones 2-5 only needed the earlier funnel
   steps. Running the full verification with those 5 still permanently unfired would only prove
   the funnel table's read side, not that the funnel itself is real — closing them first was a
   genuine prerequisite, not scope creep.
   - **`published`** — one new insert added to `approve_business_partner_request()`
     (`supabase/migrations/20260817_business_acquisition_remaining_funnel_events.sql`, pulled
     the live function body fresh via the Management API first, confirmed byte-identical to the
     last committed migration before touching it), fired from the same guarded, non-repeatable
     `status = 'pending'` match the existing `apply_approved` insert already uses, right next to
     it — there's no separate "publish" step in this app's real flow, the business goes live the
     instant an admin approves it. **Verified live against production**, not just applied: a real
     disposable pending `business_partner_requests` row (requester `Google voice`) approved by
     the real admin (`Allen`, under genuine `SET ROLE authenticated` + `request.jwt.claims`, not
     a table-owner bypass) correctly produced both a real `apply_approved` and a real `published`
     row, each with the correct real requester `user_id` and the real new `partner_id` — and
     confirmed the approve's other side effects (the new `brand_partners` row, the requester's
     `managed_partner_id`) landed correctly too. All test state (the 2 events, the
     `brand_partners` row, the request row, `managed_partner_id`) reverted afterward — confirmed
     production back to its exact pre-test baseline (1 `business_partner_requests` row — the
     real pre-existing baseline, not test residue).
   - **`first_consumer_interaction`** — built as a real, server-verified AFTER INSERT trigger on
     `business_profile_views` (`supabase/migrations/20260817_business_acquisition_first_
     consumer_interaction.sql`), not a client-side check. Deliberately server-side: a consumer
     viewing a business's profile has no honest way to know whether their view is genuinely the
     business's first-ever (`get_business_discovery_stats()` is owner-gated for exactly this
     reason, per Milestone 4's own migration) — a `NOT EXISTS` check against the same
     `business_profile_views` table Milestone 4 already built is the real, verified version of
     that signal, firing exactly once per partner on the row that's genuinely first.
     `BusinessProfileScreen.js`'s existing `logBusinessProfileView()` call (Milestone 4, already
     fires on every real consumer view, already excludes the owner's own preview) needed no
     change — the trigger observes writes that already happen. **Verified live against
     production**: a real disposable first view (a real profile, `Coastal Coffee` as the
     partner) correctly fired one `first_consumer_interaction` row with the real viewer's own
     `user_id`; a second real view from a *different* viewer on the *same* partner correctly
     fired nothing further (`count` stayed at exactly 1) — proving the "once per partner" logic
     genuinely holds, not just that it's present in the SQL text. All test rows (2
     `business_profile_views`, 1 `business_acquisition_events`) deleted afterward; production
     confirmed back to its exact pre-test baseline.
   - **`profile_completed`/`first_offer_created`/`dashboard_viewed`** — wired into
     `BusinessDashboardScreen.js`: a real `crypto.randomUUID()` per-mount `sessionId` (same
     pattern Milestone 2's apply screen already established, deliberately not threaded through
     to the earlier apply-flow session — matches Milestone 1's own disclosed scope boundary).
     `dashboard_viewed` fires in `loadMyPartner()` once a real managed partner resolves (every
     real mount, not just the first — matches the event's own name literally). `profile_completed`
     fires in `handleSaveProfile()` right after a real successful save. `first_offer_created`
     fires in `handleCreateOffer()`, guarded by `offers.length === 0` checked against the
     already-loaded offers list *before* the insert (a real, honest "is this genuinely the
     business's first offer" signal, not a post-insert re-derivation that could double-count
     under a race). **Not yet independently live-verified with a real click-through** — the
     wiring was reviewed against the exact call sites and matches the already-verified
     `published`/`first_consumer_interaction` pattern, but (unlike the two schema/RPC-level
     pieces above) this is pure client-side React state, which this sandbox can't exercise via
     direct SQL the way the other two were — flagged honestly rather than claimed proven; next
     session should confirm all three fire correctly on a real device.
   
   **Note re: baseline drift found mid-verification** — `business_acquisition_events` was found
   to already hold 2 real rows (`landing_viewed`/`cta_clicked`, `user_id: null`, same
   `session_id`) before any test data was added this pass, not the 0 Milestone 5's own cleanup
   left it at. These read as genuine anonymous traffic from the now-live `docs/business.html`
   page (GitHub Pages), not leftover test residue (no `Zzx`-style test marker, a real recent
